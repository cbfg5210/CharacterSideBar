...
public class SectionAdapter extends EasyRecyclerViewAdapter
    ...
    public int[] getItemLayouts() {
        return new int[]{R.layout.item_contact};
    }
    ...
    public void onBindRecycleViewHolder(EasyRecyclerViewHolder viewHolder, int position) {
        Contact contacts = this.getItem(position);
     ...
     * @param position   position
     */
    public void setHeaderLogic(Contact contacts, TextView headerTv, EasyRecyclerViewHolder viewHolder, int position) {
        if (position == 0) {
            this.setHeader(true, headerTv, contacts.getHeader());
        } else {
            Contact pre = this.getItem(position - 1);
    ...
    public List<EasySection> getSections() {
        ...
        for (int i = 0; i < itemCount; i++) {
            Contact contacts = this.getItem(i);
